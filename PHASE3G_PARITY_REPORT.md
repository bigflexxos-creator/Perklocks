...
| 5 | status                           | `pending | won | lost | pushed | void | partially_settled | cancelled` (canonical, Step 2) | `live | won | lost` (Writer A); `pending | won | lost | push` (plearn_) | **ambiguous**  | Remap: `live→pending`, `push→pushed`, `void→void` (NEVER→pushed). See status map in audit §8b.                                                                        |
