...
async def picks_history(
    ...
    """
    # ── MAIN 40 · Closure item #3.C (2026-06-05) — read-only History ──
    # GET /picks/history MUST NOT trigger settlement work.  Prior code
    # fire-and-forget-launched `settle_due_picks(db)` from this read
    # (guarded by a 30 s cooldown / in-flight single-flight).  Removed
    # per the directive: read routes are read-only.  Settlement
    # continues to run via the dedicated write path
    # (POST /picks/settle) and any scheduled workers.  History
    # reflects the canonical `settlement_events` + published_snapshot
    # projection captured below.
