    ...
    async def _emit_for(entry: dict, opp: str, is_home: bool) -> list[dict]:
        ...
        for route in routes:
            ...
            pick = {
                ...
                # Top-level `player_name` + `team` guaranteed on every
                # emitted pick so `player_team_fixture_validator` never
                # false-blocks with player_name_missing / player_team_invalid
                # for markets whose suffix isn't in the market-string
                # regex whitelist (e.g. "Anytime Assist").  Same fix
                # applies to `soccer_prop_inject` — see that file.
                "player_name": name,
                "player": name,
                "team": team,
                "player_team": team,
...
async def run_once() -> dict:
    ...
    try:
        ...
        try:
            ...
            pass
        # ── MAGIC 3I.1 — safe simulator reachability (producer adoption) ─
        try:
            from services.magic.direct_inject_simulator_bridge import (
                simulate_direct_inject_picks,
            )
            _sim_stats = await simulate_direct_inject_picks(db, all_picks)
            logger.info(
                "SOCCER_DIRECT_SIM producer=mls_direct_inject "
                "eligible=%d persisted=%d already=%d unsupported=%d "
                "id_blocked=%d fail=%d lock_drifts=%d",
            ...
            logger.debug("Magic 3I.1 bridge non-fatal: %s", _sim_err)
