...
async def compute_signals(db, pick: dict) -> dict:
    ...
    if isinstance(dd_contribs, dict) and dd_contribs:
        # Sum in raw percentage points but express in signal points
        # (1 pp of prob-lift ≈ 0.5 signal points, capped ±4).
        dd_total = sum(float(v) for v in dd_contribs.values() if isinstance(v, (int, float)))
        dd_points = round(max(-4.0, min(4.0, dd_total * 50.0)), 2)
        dd_why = " · ".join(
            f"{k}: {v:+.1%}"
