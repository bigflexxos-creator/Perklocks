...
# MLB hitter prop name extractor — matches the existing market labels
# generated by sports_engine ("Aaron Judge Over 0.5 Hits", "Mookie Betts
# Over 1.5 Hits", and the alt-line "Over 2.5 Hits" variants).
_MLB_PLAYER_FROM_MARKET = re.compile(
    r"^(?P<name>[A-Z][A-Za-zÀ-ÿ.\-' ]+?)\s+(?:Over|Under)\s+\d+\.\d+\s+(?:Hits|Total Bases|Strikeouts|Outs Recorded)",
    re.IGNORECASE,
)


def _extract_player(market: str, sport: str | None = None) -> str | None:
    """Extract player name from a player-prop market string.

    Handles both:
      • Soccer goalscorer markets ("X Anytime/First/Last/To Score …")
      • MLB hitter markets ("X Over/Under N.N Hits/Total Bases/Strikeouts/Outs Recorded")
    """
    if not market:
        return None
    # Soccer-first regex (works for any sport with goalscorer wording)
    m = _PLAYER_FROM_MARKET.match(market.strip())
    if not m and (sport or "").upper() == "MLB":
        m = _MLB_PLAYER_FROM_MARKET.match(market.strip())
    elif not m:
        # Try MLB fallback even when sport unspecified (e.g. legacy rows)
        m = _MLB_PLAYER_FROM_MARKET.match(market.strip())
    if not m:
        return None
...
async def recompute_auto_elite_scorers(db) -> dict:
    ...
    # Pull all settled player-prop picks chronologically (oldest first) so
    # we can compute streaks + last-N windows in a single pass. Includes:
    #   • Soccer goalscorer markets ("X Anytime/First Goal Scorer", "X To Score")
    #   • MLB hitter markets ("X Over/Under N.N Hits/Total Bases/Strikeouts/Outs Recorded")
    # Both regex extractors live in `_extract_player`, so we just need a
    # broad enough $or query to surface candidate rows.
    ...
    async for p in cur:
        ...
        player = _extract_player(p.get("market") or "", sport)
        if not player:
            continue
        k = (sport, player)
